...
n_classes = 52 # MNIST total classes (0-9 digits)
...
a=np.load("/home/zat/zresearch/ndds-corrlstm/data/sdugait/sdu.npy")
#a=a[:,270:330]
b=np.loadtxt("/home/zat/zresearch/ndds-corrlstm/data/sdugait/12023label.txt")
b=b-1
a=preprocessing.normalize(a+1) 
...
# Network Parameters
n_input = 50  #31  62 13MNIST data input (img shape: 28*28)
n_steps =  50 #50timesteps
...
#    np.savetxt('loss_lstm.csv',losslist,delimiter=',')
    ...
    np.savetxt('test_label_sdu_img'+str(round(ac,2))+'.csv',test_label)
    np.savetxt('predict_label_sdu_img'+str(round(ac,2))+'.csv',predict_relt)
    
#        sess.run(accuracy, feed_dict={x: test_data, y: test_label,rf_x: test_x, rf_y: test_y}))
    
    saver.save(sess, './modelcache/timemodel.ckpt')
    writer=tf.summary.FileWriter('./timemodel_graph',sess.graph)
    writer.flush()
    writer.close()
    sess.close()
